...
// VERSION 19 - Concurrency-limited line items + retry + maxDuration
...
//   NOTE: this file must contain the ZIGAFLOW job/section/item logic.
//   If a GET to this endpoint ever returns {"error":"Method not allowed"},
//   the Softr handler has been deployed here by mistake — redeploy this file.
...
// ---- Tunables -------------------------------------------------------------
const ITEM_CONCURRENCY = 8;   // how many addItem calls in flight at once
const ITEM_GAP_MS      = 0;   // no pause — we're racing the 60s Hobby ceiling
const MAX_RETRIES      = 2;   // retry transient (429 / 5xx / network) failures
const RETRY_BACKOFF_MS = 400; // base backoff, multiplied per attempt
// ---------------------------------------------------------------------------
...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    return res.status(200).json({
      message: 'Zigaflow Create Order - v19 (concurrency-limited + retry + maxDuration)',
